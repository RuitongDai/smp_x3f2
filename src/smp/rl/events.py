...
def init_smp_state(
  ...
  """
  del env_ids  # startup runs once over all envs
  if not ckpt_path:
    msg = (
  ...
  # Per-timestep running mean for adaptive SDS normalization (paper §5.1).
  # Count-based equal-weight average (MimicKit DiffNormalizer style).
  ...
  gsi_reset(
    env
  )  # also primes the feature buffer with current state repeated across the window
  # state = _sample_current_state(env)
  # all_ids = torch.arange(env.num_envs, device=env.device)
  # env._smp_buffer.reset(all_ids, *state)  # type: ignore[attr-defined]
...
def gsi_reset(env: ManagerBasedRlEnv, env_ids: torch.Tensor | None = None) -> None:
  ...
  # --- 1. Sample a full window (DDPM ancestral) ---
  ...
  # --- 2. Denormalize: [-1, 1] → raw feature space ---
  window = (x_t + 1.0) / 2.0 * (q_high - q_low) + q_low  # (n, W, F)

  # --- 3. Slice features ---
  base_pos_b = window[..., 0:3]  # (n, W, 3) in frame-0 yaw frame
  base_ori_6d = window[..., 3:9]  # (n, W, 6)
  base_lin_vel_b = window[..., 9:12]  # (n, W, 3)
  base_ang_vel_b = window[..., 12:15]  # (n, W, 3)
  joint_pos = window[..., 15 : 15 + J]  # (n, W, J)
  joint_vel = window[..., 15 + J : 15 + 2 * J]  # (n, W, J)

  base_quat_b = rot6d_to_quat(base_ori_6d.reshape(-1, 6)).reshape(n, W, 4)

  # --- 6. Write LAST frame's full state to sim ---
  robot = env.scene["robot"]
  last_root_state = torch.cat(
    [
      base_pos_b[:, -1],  # (n, 3)
      base_quat_b[:, -1],  # (n, 4)
      base_lin_vel_b[:, -1],  # (n, 3)
      base_ang_vel_b[:, -1],  # (n, 3)
    ],
    dim=-1,
  )  # (n, 13)
  ...
  # --- 7. Fill feature buffer with full window ---
