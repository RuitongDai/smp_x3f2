...
def x3f2_smp_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  # --- Observations --------------------------------------------------------
  actor_terms = {
  ...
  events = {
    "init_smp_state": EventTermCfg(
      ...
      params={
        # 这里先放占位路径。
        # 后面 X3_F2 prior 预训练完之后，在具体任务 cfg 里覆盖这个路径。
        "ckpt_path": "logs/pretrain/pretrained_x3f2.pt",
    ...
    "foot_friction": EventTermCfg(
      ...
      params={
        # X3_F2 每只脚 4 个小球：
        # left_foot1_collision ~ left_foot4_collision
        # right_foot1_collision ~ right_foot4_collision
        "asset_cfg": SceneEntityCfg(
          "robot", geom_names=r"^(left|right)_foot[1-4]_collision$"
        ),
        "operation": "abs",
        "ranges": (0.3, 1.2),
        "shared_random": True,
  ...
  cfg = ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      entities={"robot": get_x3f2_robot_cfg()},
      num_envs=1,
      extent=2.0,
